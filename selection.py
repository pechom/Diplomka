...
def transform_and_save(selected, prefix, path=output_dir):
    ...
    transformed_data = data[:, selected]
    with open(path + prefix + ".csv", "w", newline='') as csv_file:
        writer = csv.writer(csv_file, delimiter=',')
        writer.writerow(local_header)
        writer.writerows(transformed_data)
...
def CAT():
    model = cat.CatBoostClassifier(max_depth=7, n_estimators=100, loss_function='MultiClassOneVsAll', learning_rate=0.2,
                                   task_type='CPU', verbose=False, thread_count=4, min_data_in_leaf=10)
    print("CatBoost")
...
# def Boruta():
#     forest = RandomForestClassifier(n_estimators=100, max_depth=10, min_samples_leaf=1, max_leaf_nodes=200,
#                                     bootstrap=True, n_jobs=-1, warm_start=False)
#     selector = BorutaPy(forest, n_estimators=100, verbose=0, two_step=False)
#     print("Boruta")
#     before = datetime.datetime.now()
#     selector.fit(data, labels)
#     after = datetime.datetime.now()
#     # new_data = selector.transform(data, weak=False)
#     selected = selector.ranking_
#     print(len(selected))
